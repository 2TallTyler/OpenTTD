 ...
 * @param completed Should the house be placed already complete, instead of under construction?
 */
static void BuildTownHouse(Town *t, TileIndex tile, const HouseSpec *hs, HouseID house, uint8_t random_bits, bool completed)
{
	/* build the house */
	t->cache.num_houses++;
	...
	if (_generating_world || _game_mode == GM_EDITOR || completed) {
...
{
	if (_game_mode != GM_EDITOR && _settings_game.economy.place_houses == PHS_NONE) return CMD_ERROR;
	...
	if (flags & DC_EXEC) {
		bool completed = _settings_game.economy.place_houses == PHS_COMPLETED;
		BuildTownHouse(t, tile, hs, house, Random(), completed);
	}
