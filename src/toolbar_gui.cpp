...
static CallBackFunction ToolbarTownClick(Window *w)
{
	...
	if (_settings_game.economy.place_houses != PHS_NONE) list.push_back(MakeDropDownListStringItem(STR_SCENEDIT_TOWN_MENU_PLACE_HOUSE, TME_SHOW_PLACE_HOUSES));
...
static CallBackFunction MenuClickTown(int index)
{
	switch (index) {
		...
		case TME_SHOW_PLACE_HOUSES: // Setting could be changed when the dropdown was open
			if (_settings_game.economy.place_houses != PHS_NONE) ShowBuildHousePicker(nullptr);
			break;
	}
	return CBF_NONE;
}
